...
# Step 3: Apply different filters with varying parameters

# 1. Averaging Filter with different kernel sizes
average_variations = [cv2.blur(img, (k, k)) for k in [3, 5, 7, 9, 110]]

# 2. Gaussian Filter with different kernel sizes
gaussian_variations = [cv2.GaussianBlur(img, (k, k), 0) for k in [3, 5, 7, 9, 101]]

# 3. Median Filter with different kernel sizes
median_variations = [cv2.medianBlur(img, k) for k in [3, 5, 7, 9, 101]]

# 4. Bilateral Filter with different diameter values
bilateral_variations = [cv2.bilateralFilter(img, d, 75, 75) for d in [5, 7, 9, 11, 103]]

# Step 4: Plot the results
titles = ['Original', 'Averaging', 'Gaussian', 'Median', 'Bilateral']
variations = [average_variations, gaussian_variations, median_variations, bilateral_variations]

# Display the results for each filter with varying parameters
...
# Plot each variation for all filters
for j, (title, var_set) in enumerate(zip(titles[1:], variations), start=1):
    for i, var in enumerate(var_set):
        plt.subplot(6, 5, j + i * 5 + 1)
        plt.imshow(cv2.cvtColor(var, cv2.COLOR_BGR2RGB))
        plt.title(f'{title} k={3 + 2*i}')
        plt.xticks([]), plt.yticks([])
